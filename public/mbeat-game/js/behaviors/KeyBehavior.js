...
var KeyBehavior = function (game_object, state) {
    ...
    this.state = state;
    this.default_img = state.cache.getImage(game_object.data.default_image);
    this.alt_img = state.cache.getImage(game_object.data.alt_image);
};
...
KeyBehavior.prototype.update = function () {
    ...
    if (this.state.input.keyboard.isDown(Phaser.KeyCode.D)) {
        this.game_object.setTexture(this.alt_img);
    } else {
        this.game_object.setTexture(this.default_img);
    }
};
